...
program summa_modflow6_mpi
  ...
  ! model decisions), with SUMMA's GRUs split across MPI ranks the way summa_driver_mpi.f90 splits
  ! them for the plain (non-coupled) MPI build. MODFLOW 6 itself stays a serial singleton on rank 0:
  ! this libmf6 build has no PETSc/MPI support, and the coupled model here represents one shared
  ! aquifer grid under potentially many GRUs' HRUs, not one MODFLOW grid per GRU, so a singleton
  ! MODFLOW fed by parallel SUMMA is the natural decomposition (real MODFLOW-side parallelism would
  ! need a PETSc-enabled libmf6 and coordinating two independent domain decompositions on the same
  ...
  ! gather their local soil drainage to rank 0 (MPI_Gatherv), rank 0 alone drives MODFLOW 6 and then
  ! scatters the resulting head/storage/baseflow feedback back out (MPI_Scatterv). This file
  ! duplicates summa_modflow6.f90's MODFLOW-side subroutines (mapping, pointer helpers, projection
  ! math) verbatim, since they only ever run on rank 0 - see summa_modflow6.f90 for the serial
  ! version if only that is needed.
  !
  ...
  ! NB: no local variable in this program may be named "size" - these internal subroutines are
  ! host-associated and would then shadow the intrinsic size() used throughout the MODFLOW-side code.
  ...
  ! serial coupler; the MODFLOW-side subroutines below use them unmodified). Each rank's own SUMMA
  ! instance only ever sees its local GRU subset, so its BMI grid-0 size is nHRU_local, and its own
  ! exchange arrays are the "_local" ones - gathered into / scattered out of the global ones each
  ! step via MPI_Gatherv/MPI_Scatterv (see gather_local_to_root/scatter_root_to_local).
...
contains
  ...
  subroutine initialize_coupler
    ...
    if (rc /= 0) then
      write(*,*) 'summa_modflow6_mpi: cannot open configuration file '//trim(config_file)
      call MPI_Abort(MPI_COMM_WORLD, 1, mpi_ierr)
    end if
    read(nml=coupler, iostat=rc, unit=fu)
    close(fu)
    if (rc /= 0) then
      write(*,*) 'summa_modflow6_mpi: error reading &coupler namelist from '//trim(config_file)
      call MPI_Abort(MPI_COMM_WORLD, 1, mpi_ierr)
    end if
    if (len_trim(mf6_model_name) == 0) then
      write(*,*) 'summa_modflow6_mpi: mf6_model_name must be set in '//trim(config_file)
      call MPI_Abort(MPI_COMM_WORLD, 1, mpi_ierr)
    end if
    ...
    if (mf6_epsg /= 0) then
      ...
      if (.not. use_utm .and. myrank == 0) write(*,'(a,i0,a)') 'summa_modflow6_mpi: WARNING - mf6_epsg=',mf6_epsg, &
    ...
    if (myrank == 0) then
      ...
      head_hru = 0.0
      if (feedback) then
        allocate(bflow_hru(nHRU), stor_hru(nHRU))
      ...
      allocate(drain_hru(1), head_hru(1))
      allocate(hru_x(1), hru_y(1), hru_z(1), soil_thk(1))
      if (feedback) allocate(bflow_hru(1), stor_hru(1))
    end if
    ...
    ! ================================================================================
    ! everything below drives MODFLOW 6 itself, so it runs on rank 0 alone; the other
    ! ranks' SUMMA instances are driven entirely from run_coupler's gather/scatter calls
    ! ================================================================================
    if (myrank == 0) then

      ! -- initialize MODFLOW 6 (reads mfsim.nam from the current working directory) --
      istat = mf6_initialize()
      if (istat /= BMI_OK) then; write(*,*) 'summa_modflow6_mpi: MODFLOW 6 initialize failed'; error stop 1; end if
      ...
      ! -- optional groundwater-feedback pointers (only needed when feedback = .true.); bflow_hru/
      !    stor_hru were already allocated (size nHRU) above, alongside drain_hru/head_hru --
      if (feedback) then
        allocate(sy_hru(nHRU))
        ...
        if (len_trim(bflow_package_name) > 0) then
          ...
          if (.not. have_bflow) write(*,'(a)') 'summa_modflow6_mpi: WARNING - boundary package "'// &
      ...
      if (tend_mf6 < numtim*real(data_step, c_double)*(1.0_c_double - 1.0e-6_c_double)) then
        write(*,'(a)') 'summa_modflow6_mpi: WARNING - MODFLOW simulation is shorter than the SUMMA simulation'
      end if
    ...
    ! -- every rank needs to know whether the feedback terms are actually available, and its
    !    own local-sized buffers to set_value them (have_sy/have_bflow are decided on rank 0
    !    above, from MODFLOW's own packages) --
    if (feedback) then
  ...
  subroutine run_coupler
    do modelTimeStep = 1, numtim
      ...
      ! 3. SUMMA soil drainage -> MODFLOW 6 RCH recharge array: every rank gets its own slice,
      !    then rank 0 gathers all ranks' slices into the domain-wide array and regrids it
      istat = summa%get_value('soil_water__drainage_volume_flux', drain_hru_local)
      call gatherv_real(drain_hru_local, drain_hru)
      if (myrank == 0) call scatter_drainage_to_rch

      ! 4. advance MODFLOW 6 one coupling step (rank 0 only)
      if (myrank == 0) then
        ...
        if (modelTimeStep == 1) then
          ...
          if (abs(dt_mf6 - real(data_step, c_double)) > 1.0e-6_c_double*real(data_step, c_double)) then
            write(*,'(a,g0,a,g0,a)') 'summa_modflow6_mpi: MODFLOW time step (', dt_mf6, &
        ...
        ! 5. read the new MODFLOW state, aggregate per HRU for the next iteration (rank 0 only;
        !    scattered out to every rank at the top of the next iteration)
        if (feedback) then
  ...
  subroutine finalize_coupler
    if (myrank == 0) istat = mf6_finalize()
