...
CHARACTER(LEN=summaPathLen)  :: BASIN_ATTRIBGRID = 'none'                           ! model glacier bed topography 
CHARACTER(LEN=summaPathLen)  :: MODEL_INITGRID   = 'none'                           ! model glacier initial conditions (surface topography)
...
subroutine summa_SetTimesDirsAndFiles(summaFileManagerIn,err,message)
  ...
  do iControl=1,nControl
    ...
    case('controlVersion' );
      ...
      case('attribGridFile'     ); BASIN_ATTRIBGRID = trim(varEntry)              ! grid attribute file (glacier bed topography)
      case('initGridFile'       ); MODEL_INITGRID = trim(varEntry)                ! grid initial conditions file (glacier surface topography)
