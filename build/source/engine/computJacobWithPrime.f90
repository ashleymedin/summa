  ...
  associate(&
    ...
    do iLayer=1,nLayers
      if(ixSnLaSoIcNrg(iLayer)/=integerMissing)then
        dMat(ixSnLaSoIcNrg(iLayer)) = ( mLayerVolHtCapBulk(iLayer) + LH_fus*iden_water*mLayerdTheta_dTk(iLayer) ) * cj &
    ...
    if(enthalpyStateVec)then 
      ...
      do iLayer=1,nLayers
        if(ixSnLaSoIcNrg(iLayer)/=integerMissing) dMat(ixSnLaSoIcNrg(iLayer)) = 0._rkind
      end do
