...
subroutine snowGlceDepth(&
  ...
  tooMuchSublim=.false.  ! initialize too much sublimation (merge snow layers) to false
  ...
  if(nGlce>0)then
    if(mLayerMeltFreeze(1)>0.0_rkind)then ! check that there is no melt
      mLayerDepth(1) = mLayerDepth(1) - dt_sub*mLayerMeltFreeze(1)/iden_ice ! update the depth of the top layer (m)
      if(mLayerDepth(1)<verySmall)then ! check that we did not remove the entire layer
        mLayerDepth(1)=0.0_rkind
        message=trim(message)//'melt of glacier ice has been computed when no glacier ice exists'
        err=20; return
      end if
    end if
  end if  ! if glacier ice layers exist
...
if(nSnow==0)return ! process the case of "snow without a layer" outside of this subroutine
...
do iSnow=1,nSnow
  ...
  CR_ovrvdnPress = (weightSnow/baseViscosity)*chi4*chi5 ! compaction associated with over-burden pressure (s-1)
  ...
  if(iden_ice*mLayerVolFracIceNew(iSnow) < snwDensityMax)then ! only collapse layers if below a critical density
    volFracIceLoss = max(0._rkind,mLayerMeltFreeze(iSnow)/iden_ice)  ! volumetric fraction of ice lost due to melt and sublimation (-)
