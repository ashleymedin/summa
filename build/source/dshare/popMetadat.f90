...
subroutine popMetadat(err,message)
  ...
  flux_meta(iLookFLUX%scalarGlceMelt)                  = var_info('scalarGlceMelt'                 , 'glacier ice melt, upwards flux so negative'                       , 'm s-1'           , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  ...
  indx_meta(iLookINDEX%noWatState)            = var_info('noWatState'           , 'number of layers with no water state (bottom glacier ice layers)'        , '-', get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
